...
def draw_rad_heat_flux_curve_Fv(d_flame, height_original, layer_thickness, R_distance):
    #d_flame:火焰直径，array
...
def draw_rad_heat_flux_curve_Fh(d_flame, height_original, R_distance_max, layer_thickness,fig):
...
def draw_rad_heat_flux_vertical_view(d_flame, height_original, R_distance_max, layer_thickness, fig):
...
#需要手动设置的参数：
R_distance_max=50#这个参数是绘制曲线时x轴的范围，应大于火焰半径
layer_thickness=0.5
...
# draw_rad_heat_flux_curve_Fh(d_flame, height_original, R_distance_max, layer_thickness,fig)

#垂直圆柱体火焰热流密度分布俯视图
draw_rad_heat_flux_vertical_view(d_flame, height_original, R_distance_max, layer_thickness, fig)

#垂直圆柱体火焰伤害半径示意图
# 绘制5个不同热流对应的距离火焰的半径（5个同心圆）
# flame_hazardous_radius_xa(d_flame, height_original, R_distance_max, layer_thickness, rad_heat, fig)
...
print("ah")
# draw_rad_heat_flux_curve_Fv(d_flame, height_original, layer_thickness, R_distance)
