...
def heat_flux_v(d_flame, height_original, z_height, layer_thickness, x_distance):
    ...
    for i in range(0, layerNum):
        ...
        A2 = (h2 ** 2 + S ** 2 + 1) / (2 * S)
        # EQ(13)
        Fv1 = (1 / (pi * S)) * atan(h1 / sqrt(S ** 2 - 1)) - (h1 / (pi * S)) * atan(sqrt((S - 1) / (S + 1))) + (
                                                                                                                   A1 * h1) / (
                                                                                                                   pi * S * sqrt(
                                                                                                                       A1 ** 2 - 1)) * atan(
            sqrt(((A1 + 1) * (S - 1)) / ((A1 - 1) * (S + 1))))

        Fv2 = (1 / (pi * S)) * atan(h2 / sqrt(S ** 2 - 1)) - (h2 / (pi * S)) * atan(sqrt((S - 1) / (S + 1))) + (
                                                                                                                   A2 * h2) / (
                                                                                                                   pi * S * sqrt(
                                                                                                                   A2 ** 2 - 1)) * atan(
            sqrt(((A2 + 1) * (S - 1)) / ((A2 - 1) * (S + 1))))
        Fv = Fv1 - Fv2
        # print(Fv1)
        T = 600  # tempreture
        # T=tempCal(H)
...
def heat_flux_h(d_flame, height_original, layer_thickness, R_distance):
    ...
    for i in range(1, layerNum):
        ...
        Fh1 = (B - 1 / S) / (pi * sqrt(B ** 2 - 1)) * atan(sqrt((B + 1) * (S - 1) / (B - 1) / (S + 1))) - (
                                                                                                              A1 - 1 / S) / (
                                                                                                              pi * sqrt(
                                                                                                              A1 ** 2 - 1)) * atan(
            sqrt((A1 + 1) * (S - 1) / (A1 - 1) / (S + 1)))
        Fh2 = (B - 1 / S) / (pi * sqrt(B ** 2 - 1)) * atan(sqrt((B + 1) * (S - 1) / (B - 1) / (S + 1))) - (
                                                                                                          A2 - 1 / S) / (
                                                                                                              pi * sqrt(
                                                                                                                  A2 ** 2 - 1)) * atan(
            sqrt((A2 + 1) * (S - 1) / (A2 - 1) / (S + 1)))
